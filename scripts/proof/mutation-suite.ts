...
const MUTATIONS: Mutation[] = [
  ...
  },
  {
    ...
    from: `    HAVING MIN(f."scheduledAt") <= \${instant}`,
    to: `    HAVING TRUE AND \${instant} IS NOT NULL`,
    catcher: "src/lib/economy/economy-clock-isolation.test.ts",
  },
  {
    name: "a season becomes tier-effective when its ROW is created, not when it kicks off",
    file: "src/lib/economy/season-tier.ts",
    from: `     WHERE f."stage" = 'LEAGUE' AND f."scheduledAt" IS NOT NULL`,
    to: `     WHERE f."scheduledAt" IS NOT NULL`,
    catcher: "src/lib/economy/economy-clock-isolation.test.ts",
  },
  {
    name: "the sponsor week prices from LIVE wages instead of economic history",
    file: "src/lib/economy/weekly-settlement.ts",
    from: "      const states = await economicStatesAsOf(tx, eligibleIds, instant)",
    to: "      const states = await economicStatesAsOf(tx, eligibleIds, new Date())",
    catcher: "src/lib/economy/economy-linearization.test.ts",
  },
  {
    name: "a fixture prices its crowd at settlement time instead of at kickoff",
    file: "src/lib/match/engine/build-snapshot.ts",
    from: "  const economicInstant = fixture.scheduledAt ?? fixture.createdAt",
    to: "  const economicInstant = new Date()",
    catcher: "src/lib/economy/economy-linearization.test.ts",
  },
  {
    name: "economic history is stamped from the application clock, not the database's",
    file: "src/lib/economy/state-history.ts",
    from: "COALESCE(${domainInstant}::timestamp(3), (clock_timestamp() AT TIME ZONE 'UTC')::timestamp(3))",
    to: "COALESCE(${domainInstant}::timestamp(3), ${new Date()}::timestamp(3))",
    catcher: "src/lib/economy/economy-linearization.test.ts",
  },
  {
    name: "the as-of read falls back to current state instead of failing closed",
    file: "src/lib/economy/state-history.ts",
    from: "  if (missing.length > 0) throw new EconomicStateUnavailableError(missing, instant)",
    to: "  if (missing.length > 0) void missing",
    catcher: "src/lib/economy/economy-linearization.test.ts",
  },
  {
    name: "the as-of read loses its tie-break and returns an arbitrary same-timestamp row",
    file: "src/lib/economy/state-history.ts",
    from: `     ORDER BY s."teamId", s."effectiveAt" DESC, s."version" DESC`,
    to: `     ORDER BY s."teamId", s."effectiveAt" DESC`,
    catcher: "src/lib/economy/economy-linearization.test.ts",
  },
  {
    name: "a release stops recording that the club's wage bill fell",
    file: "src/lib/transfers/release.ts",
    from: `    await appendTeamEconomicState(tx, { teamId: input.teamId, reason: "release" })`,
    to: "    void appendTeamEconomicState",
    catcher: "src/lib/economy/economy-linearization.test.ts",
  },
  {
    name: "a salary writer stops serializing with the activation crossing",
    file: "src/lib/youth/promote.ts",
    from: "  await acquirePhase3RActivationShared(tx)",
    to: "  void acquirePhase3RActivationShared",
    catcher: "src/lib/economy/economy-linearization.test.ts",
  },
  {
    name: "the settlement reads history without excluding concurrent appends",
    file: "src/lib/economy/weekly-settlement.ts",
    from: "      await acquireEconomyHistoryExclusive(tx)",
    to: "      void acquireEconomyHistoryExclusive",
    catcher: "src/lib/economy/economy-linearization.test.ts",
  },
  {
    name: "the fixture seed goes back to being invented per attempt",
    file: "src/lib/match/simulate.ts",
    from: "  const seed = await ensureFixtureSeed(fixtureId, fixture.matchSeed)",
    to: "  const seed = fixture.matchSeed ?? generateMatchSeed()",
    catcher: "src/lib/economy/economy-linearization.test.ts",
  },
  {
    name: "the fixture seed pre-commit loses the predicate that makes it idempotent",
    file: "src/lib/match/fixture-seed.ts",
    from: `     WHERE "id" = \${fixtureId} AND "matchSeed" IS NULL`,
    to: `     WHERE "id" = \${fixtureId}`,
    catcher: "src/lib/economy/economy-linearization.test.ts",
  },
  {
